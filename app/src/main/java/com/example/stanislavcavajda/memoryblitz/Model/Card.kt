...
class Card: BaseObservable{
    ...
    fun check(v:View) {
        ...
        when(DataManager.typeSettingsActivity) {
            ...
            Constants.SETTINGS_ENDLESS -> recyclerView = (context as Activity).findViewById<RecyclerView>(R.id.endless_recycler_view)
            Constants.SETTINGS_SPEED_GAME -> recyclerView = (context as Activity).findViewById<RecyclerView>(R.id.recyclerView)
        }
